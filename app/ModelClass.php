...
class ModelClass
{
	...
	public function listHistory( $params=null )
	{
        try {
        	...
        	if ( isset($params['city_id']) ) {
        		$options.= "AND city_id=".$params['city_id']." ";
        	}
        	if ( isset($params['date']) ) {
        		$options.= "AND CAST(created_at AS DATE)='".$params['date']."' ";
        	}
        	if ( isset($params['hour']) ) {
        		$options.= "AND HOUR(created_at)='".$params['hour']."' ";
        	}
			$result = array();
	        $sql = ("
				SELECT *
				FROM weather
				WHERE id IS NOT NULL ".$options."ORDER BY name ASC, created_at DESC
	        ");
    ...
    public function insert( Array $input )
    {
        try {
	        $sql = ("
	            INSERT INTO weather (`city_id`, `name`, `country`, `timezone`, `latitud`, `longitud`, `forecast`, `description`, `icon`, `temp`, `feels_like`, `temp_min`, `temp_max`, `pressure`, `humidity`, `created_at` )
	            VALUES (:city_id, :name, :country, :timezone, :latitud, :longitud, :forecast, :description, :icon, :temp, :feels_like, :temp_min, :temp_max, :pressure, :humidity, :created_at );
	        "); 
            $query = $this->conn->prepare($sql);
            $query->execute( $input );
